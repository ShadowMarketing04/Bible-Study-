...
if [ -n "${JWT_SECRET:-}" ]; then ENV_ARGS+=(-e "JWT_SECRET=$JWT_SECRET"); fi

echo "==> deploying${VERCEL_SCOPE:+ (scope: $VERCEL_SCOPE)}"

# Move build output into .vercel/output for Vercel (and clean stale .vercel project link first)
rm -rf .vercel
mkdir -p .vercel
mv vercel-out .vercel/output
